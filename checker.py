...
class Subject:
    ...
    def check(self):
        "足りない分を計算する"
        return self.unit - self.border


class Subject_Study(Subject):
    graduation_study = 0
...
class Educational_Subject:
    ...
    def __init__(self, summary_dict):
        unit = summary_dict['基礎セミナー']
        unit += summary_dict['言語文化']
        unit += summary_dict['健康・スポーツ科学（実習）']
        unit += summary_dict['健康・スポーツ科学（講義）']
        self.basic_subject = Subject(16., unit, '全学基礎科目')

        unit = summary_dict['文系基礎科目'] + summary_dict['文系教養科目']
        self.bunkei_subject = Subject(4., unit, "文系科目")

        unit = summary_dict['理系基礎科目']
        self.science_basic_subject = Subject(23., unit, "理系基礎科目")

        unit = summary_dict['理系教養科目']
        self.science_cultural_subject = Subject(4., unit, "理系教養科目")

        unit = summary_dict['開放科目'] + summary_dict['全学教養科目']
        self.zengaku_cultural_subject = Subject(2., unit, "全学教養科目")
    ...
    def check(self):
        ...
        def append_subject(subject):
            insufficient_subjects.append((str(subject), subject.check()))

        append_subject(self.basic_subject)
        append_subject(self.bunkei_subject)
        append_subject(self.science_basic_subject)
        append_subject(self.science_cultural_subject)
        append_subject(self.zengaku_cultural_subject)

        sum = self.basic_subject
        sum += self.bunkei_subject
        sum += self.science_basic_subject
        sum += self.science_cultural_subject
        sum += self.zengaku_cultural_subject

        insufficient_subjects(('合計', sum - self.sum_unit_border))
...
def subject_table_dict(table):
    def to_tuple(tr):
        td = tr.find_all('td')
        name = td[0].text
        unit = float(td[2].text)
        return (name, unit)

    filter_null = filter(lambda tr: len(tr.find_all('td'))
                         != 0, table.find_all('tr'))
    filter_title = filter(lambda tr: tr.find_all('td')
                          [2].text != '　', filter_null)
    return dict(map(to_tuple, filter_title))
...
def get_summary_score(url):
    ...
    scores = scores_dict(tables[2])
    subjects = subject_table_dict(tables[1])
    scores.update(subjects)
...
def regulate_subjects(subject_list):
    ...
    return dict(map(regulate, subject_list))
